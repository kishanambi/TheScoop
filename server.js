...
function postComment(url, request){
  ...
  const reponse = {};
  if(commentbody && commentbody.hasOwnProperty('comment') && (commentbody.comment.body) &&
       database.articles[commentbody.comment.articleId] && database.users[commentbody.comment.username]){
    const newComment = {
      id: database.nextCommentId,
      body: commentbody.comment.body,
      username: commentbody.comment.username,
      articleId: commentbody.comment.articleId,
      upvotedBy:[],
      downvotedBy:[]
    };
...
function updateComment(url, request){
  ...
  const oldComment = database.comments[Commentid];
  ...
  if (!oldComment ) {
    ...
    response.body = {article: oldComment};
    response.status = 200;
  }
  return response;
}
...
function deleteComment(url,request){
  const CommentId_fordeletion = Number(url.split('/').filter(segment => segment)[1]);
  const currentComment_fordeletion = database.comments[CommentId_fordeletion];
  const reponse = {};
  if (!CommentId_fordeletion || !currentComment_fordeletion){
    reponse.status = 404;
  } else {
    const deleteCommentId = database.articles[currentComment_fordeletion.articleId];
    const articleCommentId = deleteCommentId.commentIds;
    const commentUsername = currentComment_fordeletion.username;
    ...
    database.comments[CommentId_fordeletion] = null;
...
function upvoteComment(url, request) {
  const CommentId_forupvote = Number(url.split('/').filter(segment => segment)[1]);
  let currentComment_forupvote = database.comments[CommentId_forupvote];
  const commentUsername = request.body && request.body.username;
  const response = {};

  if (currentComment_forupvote && database.users[commentUsername]) {
    currentComment_forupvote = upvote(currentComment_forupvote, commentUsername);
    response.body = {comment: currentComment_forupvote}
    response.status = 200;
  }
  else {
    response.status = 400;
...
function downvoteComment(url, request) {
  const CommentId_fordownvote = Number(url.split('/').filter(segment => segment)[1]);
  let currentComment_fordownvote = database.comments[CommentId_fordownvote];
  const commentUsername = request.body && request.body.username;
  const response = {};

  if (currentComment_fordownvote && database.users[commentUsername]) {
    currentComment_fordownvote = downvote(currentComment_fordownvote, commentUsername);
    response.body = {comment: currentComment_fordownvote}
    response.status = 200;
  }
  else {
    response.status = 400;
